...
Developer Functional Specification - FINAL APPROVED

**Prepared by AppZoro Technologies Inc.**

Version: Final Approved Functional Specification | Date: August 3, 2026
...
- Web Portal login roles: Employee, Admin, Super Admin, and Client. Guard users log in through the Guard Mobile App.
- Role-based redirection to the applicable dashboard.
- Access only to modules assigned through Roles and Permissions.
- Employee, Admin, Super Admin, and Client use the web portal according to assigned role/module permissions. Guard access is mobile-app only.

## 2.2 Roles and Permissions

- Users may have multiple roles.
- Roles are created through role and permission toggles.
- No separate Dispatcher or separate Supervisor account type is required. Operational supervisor responsibilities are handled by an Admin with the required site/module permissions.
...
- Assign multiple roles - Associates the selected multiple roles with the current record.
...
- Flags - Not used in the current phase; remove from active report behavior/UI where possible. Retain only if needed for source traceability.
- Date - Displays or captures the applicable calendar date.
...
- Incident Flags Only - Not used in the current phase.
- All Status - Filters the current list to records matching all status.
- Pending Approval - Filters reports waiting for Admin/Super Admin approval.
- Approved - Filters the current list to records matching approved.
- Verification - Not used; remove from current status options.
- Job Pending - Not used; remove from current status options.
- Archived - Shows records that have been archived and are no longer active.
- Date From-To - Filters records whose applicable date falls within the selected start and end dates.
- Global Search - Searches across the applicable fields of the current page and returns matching records.
...
CURRENT PHASE: Guard may Accept/Reject an assigned shift; rejection alerts Site Admin. Guard may request swap/replacement and Admin manually resolves. Later Phase: open-shift claiming/marketplace behavior.
...
- Approval requirement is configured while creating the Custom Report. If Approve Automatically is enabled, a submitted report becomes Approved immediately; otherwise it becomes Pending Approval.
- No multi-level approval workflow. A report requiring approval may be approved by an eligible Admin or Super Admin, but the report submitter cannot approve their own report.
- Authorized Admin/Super Admin may edit a submitted report. Any edit to a submitted or approved report preserves the previous version and returns the report to Pending Approval when approval is required.
- Reports are site-specific.
- Digital acknowledgment is supported.
- Report Mentions are excluded.

Approved reports are eligible for Client Portal visibility according to the Client's assigned-site access. Reports still Pending Approval are not published to the Client Portal.
...
- Region - Displays or filters by the region associated with the record.
- Description - Stores or displays explanatory details for the record.
- Level - Classification field only; it does not change approval or notification behavior in the current phase.
- Parent Category - Optional organizational relationship used to classify Incident Categories.
- Default Group - Optional classification/reference field only; no automatic routing behavior is assumed.
- Edit - Opens the existing record in editable form and saves validated changes.
- Edit Form / Sub-fields - Admin can configure the incident-category form fields. Selecting a category does not automatically open a separate Sub Form workflow.
- Filter All Groups - Filters the current list to records matching all groups.
- Filter Default Groups - Filters the current list to records matching default groups.
- Global Search - Searches across the applicable fields of the current page and returns matching records.
...
- Company Footer - Admin may upload/configure the single Company Footer used for report PDF output.
- Footer configuration is company-level only; site-specific and report-template footer precedence are not required.
...
- Edit Footer - Updates the active Company Footer. Previously generated PDFs retain the footer that existed when they were generated.
...
- Assign Forms - A form may be assigned to multiple employees; every assigned employee must complete it individually.
- Completion Status - Track submission/completion status separately for each assigned employee.

## 10.3 Form Submissions

Each employee may submit an assigned form once. Draft/edit is allowed only before submission. After submission, the employee cannot edit or resubmit. Preserve submitted values if the template is later archived/deleted; no repeat-submission history/version chain is required.
...
Training implementation is defined in Part II, Section 42. Certificates, expiration and automatic skill linkage are not required unless separately configured through Skills.
...
Automation scope is site-based. A rule may be assigned to one site or multiple selected sites; no separate company-global scope is required unless later requested.
...
- Group fields: Group Name, Description, Members, Status, and Activity.
- Create Group - Allows Admin to create a specific-purpose manual collection of users.

CURRENT PHASE: Group is a manually created specific-purpose collection of selected people. One employee may belong to multiple Groups. Groups may be used for activity/communications such as Broadcasts, Send an Update, and applicable Automation targeting. Groups do not control Schedule access; Schedule visibility follows assigned Site access. Department remains the permanent organizational team. No separate rule-based Segment behavior is required.
...
- Operational supervisor responsibilities are performed by an Admin user in the same web portal; no separate Supervisor account type is required.
- Admin access is controlled by assigned roles, module permissions, and assigned-site scope.
- When site-restricted, Admin can access permitted modules and records only for assigned sites.
- No separate Dispatcher or separate Supervisor account type is required. Operational supervisor responsibilities are handled by an Admin with the required site/module permissions.
...
- Current phase: Guard shift acceptance/rejection and guard-requested swap/replacement are supported with Admin notification/manual action. Later Phase: open-shift claiming/marketplace behavior.
...
- Multiple-client site relationship is resolved: one Primary Client may be marked as an indicator only; assigned clients can access minimum site reports/schedules/employees and see other assigned clients; no shared-site financial responsibility logic.
...
- Later Phase: TrackTik/Connecteam historical migration, parallel operation, cutover and historical-data categories.
...
| Clients and sites                 | Confirmed including shared-site minimum visibility and closure logic; Contracts and remaining Client Portal details later phase                    |
| Scheduling                        | Confirmed with pending shift-marketplace behaviors                                                                                                 |
| Time clock and work exceptions    | Confirmed core logic; detailed Break Management and offline synchronization later phase                                                            |
| Reports and incidents             | Custom report configuration confirmed; TrackTik-style runtime statuses/flags/incident logic/numbering/footer precedence still require confirmation |
| Forms                             | Confirmed                                                                                                                                          |
| Checkpoints and tours             | Confirmed                                                                                                                                          |
| Tasks and dispatch                | Confirmed                                                                                                                                          |
| Communications                    | Confirmed for current scope; Admin private-chat visibility and Chat Settings rules resolved                                                        |
| Security Operations landing area  | Confirmed                                                                                                                                          |
| Team Resources                    | Source-listed; detailed HR workflows pending                                                                                                       |
| Training                          | Current-phase Quizzes and RFI Academy logic defined in Part II                                                                                     |
| Vehicles                          | Documentation only                                                                                                                                 |
| Automations                       | Confirmed: site/multi-site event builder, 30-minute duplicate suppression, In-App/Push/SMS/Create Task actions                                     |
| Payroll                           | Confirmed: policy/pay-rule auto-calculation logic; integrations later phase                                                                        |
| Settings                          | Confirmed and rationalized in Part II; unsupported source settings removed from current phase                                                      |
| Groups / Job List                 | Groups confirmed as manual specific-purpose collections; separate Job List excluded and covered by Task List/Job Type                              |
...
- The Position/Post configuration defines the inactivity-check interval. Inactivity notification/ticket remains open until manually resolved by Admin; do not auto-close it.
...
- Employee Status values are: Pending Invitation, Active, Inactive, and Terminated.
- Pending Invitation = employee created and invitation sent but not yet accepted; the employee cannot be assigned to a site or shift. Active = invitation accepted and employee is eligible for site/shift assignment. Inactive = manually made unavailable; login is disabled and existing site assignments are removed. Terminated = employment ended by Admin and access is revoked.
- Email address must remain unique even when multiple employees use the same Employee Type.
- Role/module permissions define web access. Employee, Admin, Super Admin, and Client may log in to the web portal; Guard users use the mobile app. Only Super Admin can grant Admin rights to an Employee. Employee web access is limited to permitted modules/data; Admin receives assigned-site edit access plus explicitly granted modules; Super Admin has full platform access.
...
- Expiring-soon alert days are configurable while creating the Skill/Skill Category.
- Add a Soft Skill setting. Expired required/hard skills block new Site/Shift assignments; expired Soft Skills generate warning only and do not block assignment.
- If a required Skill expires while the guard is already on an active shift, allow the current shift to continue but block new/future assignments requiring that Skill.
- If Admin rejects an uploaded certificate, the employee may upload a replacement for the same Skill assignment. Preserve rejected files/status in the employee Skill/Certification audit/history.
...
- List ID, Type, Date, Reported By, Account, Status, Printable PDF, Email Report, View and Remove. Flags are not used in the current phase.
- Apply Active/All Templates/Archived, Status, From-To Date and Global Search filters. Incident Flags Only is excluded from current scope.
...
- One client may be marked Primary for a shared Site; Primary is an indicator only and does not change permissions, visibility, billing, or workflow.
...
- Only Super Admin may remove a Client-Site assignment when other clients remain. Removal immediately revokes the Client's current access to that Site, its reports, schedules, and employee information while preserving historical access/activity records.
- For a Client assigned to multiple Sites, the Client Portal dashboard header provides a Site selector dropdown for switching among assigned Sites.
...
- Custom Position/Job Type forms may include Admin-defined custom fields in addition to the source-listed fields.
...
- Tour is created separately for a site and is assigned to scheduled employees matching the tour site/date/time; it is not created as a child of the shift. The first scheduled guard is auto-assigned first; additional guards may be manually assigned.
- Multiple tours may be assigned within one shift. The same tour template cannot be repeated twice in the same shift.
- When multiple guards are assigned to one Tour occurrence, each guard has an independent Tour session and completion state.
...
- The guard may see the expected randomized Tour time/window in advance.
...
- If an Interrupted Tour is never resumed before shift end, its final status remains Interrupted.
- When a High Priority Task interrupts a Tour, pause the Tour timer. On task completion, automatically return the guard to the interrupted Tour and resume from existing progress. Exclude interruption duration from late/incomplete calculations.
...
- A copied shift is a separate day-specific shift record after duplication and copies Job Type, User, Notes, Tags, Tasks, and Tour association by default.
...
- A shift may remain unassigned while Draft. If still unassigned before its scheduled date/time, send an Admin alert/reminder.
...
- If a published shift employee later becomes Inactive or Terminated, do not automatically remove the assignment; alert Admin for manual resolution.
- If a required Skill expires after a future shift is already published, keep the existing assignment but alert Admin. Expired required skills block new assignments.
...
- Guard can Accept or Reject an assigned shift in the Mobile App. Rejection sends an alert to the Site Admin. Guard may request a swap/replacement from Mobile; Admin receives the request and manually takes action. Open-shift claiming remains Later Phase.
...
- Use one polygon geo-fence per Site covering the permitted Site area. Save the guard's geo coordinates at clock-in.
- Clock-in is blocked when GPS permission is denied, GPS is off, guard is outside the Site polygon, or guard has no active assignment to that Site.
...
- If a guard fails to clock out at the assigned shift end, create a notification/ticket and automatically clock the guard out after the confirmed 10-minute allowance. Auto clock-out occurs even if the guard remains inside the Site polygon.
...
- Admin resolves the auto-clock-out notification/ticket manually.
- After auto clock-out, the guard may continue using the Mobile App to complete pending Tasks, Tours, and operational actions; clocked-in/payable time and app access are separate states.

## 35.2 Timesheets and Manual Changes

**Final Confirmed Logic**

- Employee cannot edit the timesheet. Admin reviews/edits/approves the timesheet; every Admin edit is audited. Super Admin may reopen a fully approved/closed timesheet.
...
- Admin may edit time records; a reason is mandatory, the employee is notified, and every change is audited.
- A manual time edit does not immediately recalculate payroll. After approval, the next automatic Payroll calculation cycle uses the approved time values.
- Only Super Admin may reopen a fully approved/closed payroll-period time record; reopening resets the approval state and requires re-approval.
...
- List ID, Type, Date, Reported By, Account, Status, Printable PDF, Email Report, View and Remove. Flags are not used in the current phase.
- Apply Active, All Templates, Archived, Status, From-To Date and Global Search filters. Incident Flags Only is not used.
- Use only report statuses Pending Approval, Approved, and Archived.
- Generated Report ID format is #00001, #00002, etc. The sequence never resets.

## 36.3 Approval and Publication

**Final Confirmed Logic**

- Custom Report setup controls whether approval is required through Approve Automatically.
- When manual approval is required, an eligible Admin or Super Admin may approve the report; the submitter may not approve their own report.
- A submitted report may be manually edited by an authorized Admin/Super Admin. Every edit preserves the prior version and returns the report to Pending Approval when approval is required.
- Admin may add notes and create an amended/replacement version while retaining the original version history.
- Report runtime logic is confirmed: Pending Approval -> Approved -> Archived. Auto-approved reports become Approved immediately. Approved reports may be exposed to the Client Portal for assigned sites; Pending Approval reports are not published. Report flags are not used.
...
- Generated Report ID uses a non-resetting sequential format beginning #00001.
...
- Report flags, Verification and Job Pending are excluded. Incident Category fields are classification/form-configuration fields only. Use one Company Footer; generated PDFs retain the footer version used at generation time.
...
- A form may be assigned to multiple employees; every assigned employee must complete the form individually.
- Form creator defines site behavior, due date, draft/edit capability, and approval requirement as configurable form settings. Repeat submission is not allowed for the same employee assignment.
...
- Each assigned employee receives an individual completion/submission status and may submit the assigned form once.
- When drafts are enabled, save partial data without treating it as submitted. Editing is allowed only before submission. After submission, the employee cannot edit or submit again.
- Submitted form data remains accessible even if the form template is later archived/deleted, but no repeat-submission history/version chain is required.
...
- High Priority: immediately interrupts normal workflow and active Tour. Emergency features (Panic Button, Emergency Contacts, Calls, Supervisor/Admin Messages) remain accessible. The Tour timer pauses and the app returns to that Tour automatically after task completion.
- Medium Priority: if a Tour is in progress, allow it to finish first. When multiple Medium tasks are waiting, execute them in due-time order.
- Low Priority: must be completed before guard clock-out. Guard clock-out is blocked while Low tasks remain incomplete; an Admin may override with mandatory reason and the system creates an exception/ticket.
...
- Admin may manually mark a Task Completed on behalf of the guard. Record an audit entry identifying the Admin, task, previous status, completion timestamp, and manual-completion action.
- A task may be reassigned to another eligible user. Preserve original assignee, previous progress, timestamps, status history, and reassignment history.
...
- Vehicle Status values are Active and Inactive. Vehicle ID is system auto-generated. VIN and License Plate must be unique.
- Inactive Vehicles are excluded from Active filters but remain historically accessible.
...
- Provide custom event-based Automation builder. Every Automation must be assigned to one or more selected existing Sites; no automatic All Sites/future-sites scope.
...
- If the same manually reported event type/context occurs within 30 minutes of a previous matching event notification/ticket, treat it as a possible duplicate.
- For a manually reported possible duplicate, warn that a matching event/ticket exists within the last 30 minutes and allow the reporter to confirm creation as a separate event or cancel.
- If confirmed as a separate manual event, create the new event/ticket; if cancelled, suppress it. Automatically generated alerts do not require Guard/Admin approval and execute directly under the active Automation rule.
- For failed SMS, Push Notification, or Create Task execution, create an in-app notification to Super Admin describing the failed action/event.
...
- Payroll calculation uses Admin-approved timesheet data. Super Admin may reopen approved/closed time records; reopened records require approval again before calculation.
...
- Admin users perform supervisor functions in the same web application and see only granted modules and assigned-site data.
- Admin may review/edit/approve timesheets for employees in permitted sites and may approve site reports when the report requires manual approval. Super Admin may perform these actions across all sites.
- Admin can create/assign forms and operational tasks within permitted sites according to module access.
...
- Later Phase / still pending: open-shift claiming/marketplace behavior; detailed Contract module; Break Management calculations/penalties; offline Guard attendance synchronization; advanced Client Portal functions beyond confirmed site selector/minimum access; Team Resources/HR modules; data migration; accounting/payroll-provider integration; Remote Speak; Runsheet Patrol Events; Daily Health; Cross Schedule Events; Financial by Shift; Recordings; detailed System Exceptions; Pivot Chart View/Edit.
- Custom Report runtime logic is resolved for current scope: statuses are Pending Approval, Approved, Archived; submitter cannot self-approve; flags are not used; Incident Categories are classification/form configuration only; Approved reports may publish to Client Portal by Site access; Report ID is sequential #00001 and never resets; one Company Footer is used.
- Do not create unsupported workflows for Later Phase items.

| **Area**                  | **Pending Decisions / Development Hold**                                                                                                                                                                                                                                                                   |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Scheduling                | Shift acceptance/rejection, open shifts, claiming, swaps and replacement workflow only. Cross-midnight ownership is resolved: shift belongs to start date.                                                                                                                                                 |
| Site/Client               | Contracts and remaining Client Portal actions/details. Shared-site minimum visibility and site-closure behavior are resolved.                                                                                                                                                                              |
...
| Communications            | No current-scope communication logic remains pending; later expansion may add further Client communication if requested.                                                                                                                                                                                   |
| HR/Team Resources         | Hiring depth, benefits, rewards/tokens, celebrations, complaints and disciplinary workflow.                                                                                                                                                                                                                |
| Training                  | No core current-phase training logic pending; certificates/expiry/automatic skill linkage are outside current scope unless configured through Skills.                                                                                                                                                      |
| Finance                   | Invoice generation and accounting/payroll-provider integrations only. Payroll formulas/policy logic are resolved.                                                                                                                                                                                          |
| Migration                 | TrackTik/Connecteam migration, historical scope, parallel operation and cutover.                                                                                                                                                                                                                           |
| Undefined source features | Later Phase: Remote Speak, Runsheet Patrol Events, Daily Health, Cross Schedule Events, Financial by Shift, Recordings, detailed System Exceptions and Pivot Chart View/Edit. Devices/License, Calendar Groups, Back Office Defaults and separate Job List are excluded. Journal Entries is current phase. |
...
End of Developer Functional Specification
...
- All RFI-confirmed logic changes, exclusions and Pending Discussion items - Represents the all rfi-confirmed logic changes, exclusions and pending discussion items value or option required by this section.
...
• Custom Report runtime behavior is now resolved for current scope: statuses Pending Approval, Approved and Archived; no report flags; Incident Categories are classification/form configuration only; Approved reports may publish to Client Portal by Site access; Report ID is sequential #00001 with no reset; one Company Footer is used.

• Other Later Phase items remain intentionally unimplemented and are not considered missing current-scope functionality.

# 51\. Final Approved Logic Consolidation

- Portal roles: Employee, Admin, Super Admin and Client use the Web Portal; Guard uses the Mobile App. Only Super Admin may grant Admin rights. No separate Supervisor account type is required; supervisor duties are performed by an Admin with site/module permissions.
- Employee lifecycle: Pending Invitation -> Active after invite acceptance; Pending Invitation cannot be assigned to Site/Shift. Inactive disables login and removes Site assignments; reactivation requires manual reassignment. Terminated revokes access and preserves history.
- Reports: statuses are Pending Approval, Approved and Archived only. Auto-approved reports become Approved immediately. Submitter cannot self-approve. Admin/Super Admin approves when required. Editing a submitted/approved report preserves prior version and returns it to Pending Approval when approval is required.
- Report flags are not used. Incident Categories classify incident types and configure form sub-fields; Level does not change notification/approval. Report ID is sequential #00001 and never resets. One Company Footer is used; generated PDFs retain the footer active at generation.
- Forms: every assigned employee must complete independently; one submission per employee assignment; edit only before submission; no repeat-submission history/version chain.
- Scheduling: copied shifts duplicate Job Type, User, Notes, Tags, Tasks and Tour association. Published employee becoming Inactive/Terminated or future required Skill expiry triggers Admin alert rather than automatic unassignment. Draft unassigned shift sends reminder before scheduled time.
- Guard can Accept/Reject shifts; rejection alerts Site Admin. Guard can request swap/replacement; Admin manually resolves. Open-shift claiming remains Later Phase.
- Tours: each assigned guard gets an independent Tour session. First scheduled guard is auto-assigned first. High Priority Task pauses Tour timer; interruption duration is excluded; app returns to Tour after task completion; unresumed Tour ends as Interrupted.
- Tasks: emergency features remain available during High Priority Task. Medium tasks are handled in due-time order after active Tour. Low tasks block guard clock-out unless Admin overrides with reason and creates exception/ticket. Admin manual completion and reassignment are fully audited.
- Timesheets: Employee cannot edit. Admin reviews/edits/approves with audit. Super Admin can reopen approved/closed time; reopening resets approval. Payroll uses approved time on the next automatic calculation cycle.
- Automatic clock-out: if Guard fails to clock out at assigned shift end, create ticket/notification and auto clock-out after 10 minutes even if still inside Site polygon. Guard may continue completing pending Tours/Tasks after auto clock-out; app access is separate from payable clocked-in time.
- Skills: Expiring Soon alert days configurable; Soft Skill does not block assignment; expired required Skill blocks new assignments but does not remove an active shift. Rejected certificate may be replaced and rejected file remains in audit/history.
- Geo-fence/inactivity: one polygon per Site; clock-in coordinates are stored. Static Position inactivity uses required app/input activity; Moving Position also tracks geo-coordinate activity. Inactivity notification/ticket requires manual Admin resolution.
- Automations: assigned to selected Sites. Manual duplicate events within 30 minutes require reporter confirmation to create a separate event. Auto-generated alerts never require approval. Failed SMS, Push, or Create Task notifies Super Admin.
- Client/Site: Primary Client is indicator only. Removing a Client from Site revokes current Site/report/schedule/employee access while preserving history. Multi-site Client Portal uses a Site selector dropdown.
- Vehicles: Active/Inactive statuses; auto-generated Vehicle ID; unique VIN and License Plate; inactive vehicles remain historically accessible.
- Groups: Group Name, Description, Members, Status and Activity; employee may belong to multiple Groups; Groups support activity/communications and applicable automation targeting, not Schedule access.

## Final Clarification Rule

This Final Approved version supersedes earlier contradictory wording in prior drafts. Only items still explicitly marked Later Phase/Pending in this document may remain unimplemented. Developers must not reintroduce superseded statuses, flags, approval chains, role types, geo-fence rules, or other earlier assumptions.