...
export function CreateTourWizard({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6"
      ...
      <div className="rounded-2xl overflow-hidden flex flex-col bg-white w-full shadow-2xl h-[90vh]"
        style={{ maxWidth: 900 }}>
        ...
        <div className="flex flex-col shrink-0" style={{ borderBottom: "1px solid #e2e8f0" }}>
          <div className="flex items-center justify-between px-6 pt-5 pb-4">
            <div>
              <h2 className="text-xl font-bold text-slate-900">Create Tour Route</h2>
              <p className="text-sm text-slate-500 mt-1">Configure the tour location, checkpoints, assignment, and recurring schedule.</p>
            </div>
            <button onClick={handleClose} className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        {/* Footer */}
        <div className="shrink-0 p-5 bg-white flex justify-between items-center" style={{ borderTop: "1px solid #e2e8f0" }}>
          <button onClick={handleClose} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50 transition-colors">
            Cancel
          </button>
          <div className="flex gap-3">
            {step > 1 && (
              <button onClick={() => setStep(step - 1 as 1|2|3)} className="px-5 py-2.5 rounded-xl text-sm font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50 transition-colors">
                Back
              </button>
            )}
            {step < 3 ? (
      ...
      {showDiscard && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <h3 className="text-lg font-bold text-slate-900 mb-2">Discard tour setup?</h3>
            <p className="text-sm text-slate-500 mb-6">Your unsaved changes will be lost.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowDiscard(false)} className="px-4 py-2 rounded-xl text-sm font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50">
                Keep Editing
              </button>
    ...
    return (
      <div className="space-y-6 max-w-3xl mx-auto pb-10">
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-5 shadow-sm">
          <h3 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-4">Basic Details</h3>
          
          <div>
            <label className="block text-sm font-semibold mb-1.5 text-slate-700">Tour Name <span className="text-red-500">*</span></label>
            <input type="text" value={tourName} onChange={e => { setTourName(e.target.value); setHasChanges(true); }}
              ...
              className="w-full px-4 py-2.5 rounded-xl text-sm border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all" />
            <p className="text-xs text-slate-500 mt-1.5">Use a name that is easy for guards and supervisors to recognize.</p>
          </div>

          <div>
            <label className="block text-sm font-semibold mb-1.5 text-slate-700">Site <span className="text-red-500">*</span></label>
            <select value={site} onChange={e => { setSite(e.target.value); setHasChanges(true); }}
              className="w-full px-4 py-2.5 rounded-xl text-sm border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none bg-white appearance-none cursor-pointer">
              <option value="" disabled>Select site...</option>
              {MOCK_SITES.map(s => <option key={s.name} value={s.name}>{s.name}</option>)}
            </select>
            {site && (
              <div className="mt-2 p-3 bg-slate-50 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between border border-slate-100 gap-2">
                <div className="flex items-center gap-2 text-sm text-slate-700">
                  <Building2 className="w-4 h-4 text-slate-400" />
                  <span className="font-medium text-slate-900">{selectedSiteObj?.name}</span>
          ...
          <div>
            <label className="block text-sm font-semibold mb-1.5 text-slate-700">Assigned To <span className="text-red-500">*</span></label>
            ...
            {assignedType === "Position" && (
              <select className="mt-3 w-full max-w-sm px-4 py-2.5 rounded-xl text-sm border border-slate-200 bg-white cursor-pointer">
            ...
            {assignedType === "Specific Employee(s)" && (
              <div className="mt-3 space-y-2">
                <select className="w-full max-w-sm px-4 py-2.5 rounded-xl text-sm border border-slate-200 bg-white cursor-pointer"
                  onChange={e => {
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-5 shadow-sm">
          <h3 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-4">Timing & Instructions</h3>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-slate-700">Estimated Duration <span className="text-red-500">*</span></label>
              <div className="flex items-center">
                <input type="number" value={duration} onChange={e => { setDuration(e.target.value); setHasChanges(true); }}
                  className="w-24 px-4 py-2.5 rounded-l-xl border border-r-0 border-slate-200 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                <div className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-r-xl text-sm text-slate-500 font-medium">Minutes</div>
              </div>
            </div>
            <div>
              <label className="block text-sm font-semibold mb-1.5 text-slate-700">Grace Period <span className="text-red-500">*</span></label>
              <div className="flex items-center">
                <input type="number" value={gracePeriod} onChange={e => { setGracePeriod(e.target.value); setHasChanges(true); }}
                  className="w-24 px-4 py-2.5 rounded-l-xl border border-r-0 border-slate-200 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
                <div className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-r-xl text-sm text-slate-500 font-medium">Minutes</div>
              </div>
              <p className="text-xs text-slate-500 mt-1.5">Supervisors are alerted when the tour exceeds this grace period. Default: 15 minutes</p>
            </div>
          </div>

          <div>
            <label className="block text-sm font-semibold mb-1.5 text-slate-700">Special Instructions</label>
            <textarea value={instructions} onChange={e => { setInstructions(e.target.value); setHasChanges(true); }} rows={2}
              ...
              className="w-full px-4 py-3 rounded-xl text-sm border border-slate-200 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all" />
          </div>
          ...
          <div>
            ...
            <select value={status} onChange={e => { setStatus(e.target.value as "Active"|"Inactive"); setHasChanges(true); }}
              className="w-full max-w-xs px-4 py-2.5 rounded-xl text-sm border border-slate-200 bg-white outline-none cursor-pointer">
    ...
    return (
      <div className="space-y-6 max-w-5xl mx-auto pb-10">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[500px]">
            <div className="p-4 border-b border-slate-100 shrink-0">
              <h3 className="text-sm font-bold text-slate-900 mb-3">Available Checkpoints</h3>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input type="text" value={cpSearch} onChange={e => setCpSearch(e.target.value)}
                  placeholder="Search checkpoints..."
                  className="w-full pl-9 pr-3 py-2.5 rounded-xl text-sm border border-slate-200 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all" />
              </div>
            </div>
            
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
              {availCps.map(cp => (
                <div key={cp.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 group transition-colors">
                  <div>
                    <div className="text-sm font-semibold text-slate-800">{cp.name}</div>
                    <div className="text-xs text-slate-500 mt-0.5">{cp.type} • Active</div>
                  </div>
              ...
              {availCps.length === 0 && (
                <div className="p-8 text-center text-sm text-slate-500">No checkpoints found.</div>
              )}
            </div>
            ...
            <div className="p-4 border-t border-slate-100 shrink-0">
              <button onClick={() => { setSelectedCheckpoints([...selectedCheckpoints, ...availCps]); setHasChanges(true); }}
                ...
                className="w-full px-4 py-2.5 rounded-xl text-sm font-bold text-slate-700 border border-slate-200 hover:bg-slate-50 disabled:opacity-50 transition-colors">
                Add All Displayed
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col h-[500px]">
            <div className="p-4 border-b border-slate-100 shrink-0 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-slate-900">Selected Route</h3>
                <p className="text-xs text-slate-500 mt-0.5">{selectedCheckpoints.length} checkpoints • Est. {duration} min</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {selectedCheckpoints.map((cp, idx) => (
                <div key={cp.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-white shadow-sm group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-slate-800 truncate">{cp.name}</div>
                    <div className="text-xs text-slate-500 truncate">{cp.type}</div>
                  </div>
              ...
              {selectedCheckpoints.length === 0 && (
                <div className="p-8 flex flex-col items-center justify-center text-center h-full">
                  ...
                  <p className="text-sm font-semibold text-slate-600">No checkpoints selected</p>
                  <p className="text-xs text-slate-500 mt-1">Select checkpoints from the left panel to build your route.</p>
                </div>
              )}
            </div>
          </div>
    ...
    return (
      <div className="space-y-6 max-w-4xl mx-auto pb-10">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 shadow-sm">
          ...
          <p className="text-lg font-medium text-slate-800">{scheduleText}</p>
        </div>

        <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6 shadow-sm">
          <h3 className="text-lg font-bold text-slate-900 border-b border-slate-100 pb-4">Schedule Details</h3>
          
          <div>
            <label className="block text-sm font-semibold mb-2 text-slate-700">Recurrence</label>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="bg-slate-50 p-5 rounded-xl border border-slate-100 space-y-5">
            {recurrence === "Weekly" && (
              <div>
                <label className="block text-sm font-semibold mb-2 text-slate-700">Repeat on <span className="text-red-500">*</span></label>
                <div className="flex flex-wrap gap-2">
            ...
            <div>
              <label className="block text-sm font-semibold mb-2 text-slate-700">Tour Time(s) <span className="text-red-500">*</span></label>
              <div className="space-y-3">
                {tourTimes.map((time, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    <input type="time" value={time} onChange={e => handleUpdateTourTime(idx, e.target.value)}
                      className="px-4 py-2.5 rounded-xl text-sm border border-slate-200 outline-none focus:border-blue-500 bg-white shadow-sm" />
                    {tourTimes.length > 1 && (
            ...
            <div>
              <label className="block text-sm font-semibold mb-2 text-slate-700">Start Date <span className="text-red-500">*</span></label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
            <h3 className="text-sm font-bold text-slate-900 border-b border-slate-100 pb-3 mb-4">Review Tour</h3>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-500">Name</span>
                <span className="font-semibold text-slate-900">{tourName || "—"}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-900">{selectedCheckpoints.length} ({checkpointOrder})</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Assigned</span>
                <span className="font-semibold text-slate-900">{assignedType}</span>
              </div>
              <div className="flex justify-between">
