    ...
    <div className="flex-1 flex flex-col min-h-0 bg-slate-50 p-6">
      ...
      <div className="flex-1 overflow-auto bg-white border border-slate-200 shadow-sm rounded-xl flex flex-col">
        <table className="w-full text-left text-sm whitespace-nowrap min-w-[900px]">
          <thead className="bg-slate-50/80 border-b border-slate-200 text-slate-500 sticky top-0 z-10">
            <tr>
              ...
              return (
                <tr key={update.id} className="hover:bg-slate-50/50 transition-colors group cursor-pointer">
                  <td className="px-5 py-4">
                    <div className="font-bold text-slate-800">{update.title}</div>
                  </td>
                  <td className="px-5 py-4 text-slate-600 font-medium">{update.audience}</td>
                  <td className="px-5 py-4 text-slate-600">{update.publishedBy}</td>
                  <td className="px-5 py-4 text-slate-500">{new Date(update.publishedDate).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</td>
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <span className="text-xs font-bold text-slate-600 shrink-0 w-12 text-right">
