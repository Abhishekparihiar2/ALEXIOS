...
export function BroadcastsTab() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-slate-50 p-6">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="w-64 pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 transition-all shadow-sm"
            />
          </div>
          <button className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 text-sm font-semibold rounded-lg transition-colors shadow-sm">
      ...
      <div className="flex-1 overflow-auto bg-white border border-slate-200 shadow-sm rounded-xl flex flex-col">
        <table className="w-full text-left text-sm whitespace-nowrap min-w-[800px]">
          <thead className="bg-slate-50/80 border-b border-slate-200 text-slate-500 sticky top-0">
            <tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {MOCK_BROADCASTS.map(broadcast => (
              ...
              >
                <td className="px-5 py-3 font-bold text-slate-800">{broadcast.title}</td>
                <td className="px-5 py-3 text-slate-600 font-medium">{broadcast.audience}</td>
                <td className="px-5 py-3 text-slate-600">{broadcast.sentBy}</td>
                <td className="px-5 py-3 text-slate-500">{new Date(broadcast.sentDate).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</td>
                <td className="px-5 py-3 font-semibold text-slate-700">{broadcast.delivery}</td>
