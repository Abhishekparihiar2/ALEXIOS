...
export function NewGroupDrawer({ isOpen, onClose }: NewGroupDrawerProps) {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-full max-w-[500px] bg-slate-50 shadow-2xl z-40 flex flex-col animate-in slide-in-from-right duration-300">
        <div className="px-6 py-5 bg-white border-b border-slate-200 flex items-center justify-between">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-slate-500">Create a group chat for a team or shift.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
          ...
          <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-sm flex-1 flex flex-col min-h-[400px]">
            <label className="text-sm font-semibold text-slate-700">Members <span className="text-red-500">*</span></label>
            <div className="flex gap-2">
              <select className="flex-1 px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium focus:outline-none focus:border-blue-500">
            ...
            <div className="flex-1 overflow-y-auto border border-slate-100 rounded-lg mt-2">
               {/* Mock list of users to select */}
               {[1, 2, 3].map((i) => (
                <label key={i} className="w-full text-left p-3 border-b border-slate-50 hover:bg-slate-50 transition-colors flex items-center gap-3 cursor-pointer">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="font-semibold text-slate-800 text-sm">Employee Name {i}</span>
                    <span className="text-xs text-slate-500 truncate">Security Guard</span>
                  </div>
                </label>
              ))}
            </div>
        ...
        <div className="px-6 py-4 bg-white border-t border-slate-200 flex justify-end gap-3 shrink-0">
          <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50 rounded-lg transition-colors">
            Cancel
          </button>
