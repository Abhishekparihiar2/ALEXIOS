...
export function SkillDrawer({ isOpen, onClose }: SkillDrawerProps) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="px-6 py-5 bg-white border-b border-slate-200 shrink-0 flex items-center justify-between sticky top-0 z-10">
          <div className="flex flex-col gap-1">
            <h2 className="text-xl font-bold text-slate-800">Add Skill / Certification</h2>
          ...
          <button 
            onClick={onClose}
            className="p-2 rounded-full hover:bg-slate-100 text-slate-500 transition-colors shrink-0 ml-4 self-start"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700">Skill / Certification Name <span className="text-red-500">*</span></label>
                <input 
                  type="text" 
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  placeholder="e.g. GA 24-Hour Guard Certification" 
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea 
                  ...
                  rows={3}
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all resize-none"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700">Reference / Verification URL</label>
                <div className="relative">
                  <input 
                    ...
                    className="w-full pl-3 pr-10 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                  />
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm font-semibold text-slate-700">This certification expires</span>
              ...
              {expires && (
                <div className="pl-13 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="space-y-1.5">
                    <label className="text-sm font-semibold text-slate-700">Default Validity Period</label>
                    <div className="flex gap-2">
                      <input 
                        type="number" 
                        value={validityValue}
                        onChange={(e) => setValidityValue(e.target.value)}
                        className="w-20 px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                      />
                      <select 
                        value={validityUnit}
                  ...
                  <label className="flex items-center gap-3 cursor-pointer group pt-2">
                    ...
                    <span className="text-sm text-slate-700">Enable expiration reminders</span>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-semibold text-slate-700">Require certificate/document upload</span>
            ...
            <div className="space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <div className="flex gap-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-slate-700">Specific Region</span>
                  <input type="radio" className="hidden" checked={scope === "Specific Region"} onChange={() => setScope("Specific Region")} />
                </label>
              </div>

              {scope === "Specific Region" && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-300 pt-2 border-t border-slate-100">
                  <label className="text-sm font-semibold text-slate-700 mb-1.5 block">Select Region</label>
                  <select 
                    value={region}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              <label className="flex items-center gap-3 cursor-pointer group">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-semibold text-slate-700">Visible to Client Portal</span>
                  <span className="text-xs text-slate-500 mt-0.5 leading-snug">
        ...
        {/* Footer */}
        <div className="px-6 py-4 bg-white border-t border-slate-200 shrink-0 flex items-center justify-end gap-3">
          <button 
            onClick={onClose}
