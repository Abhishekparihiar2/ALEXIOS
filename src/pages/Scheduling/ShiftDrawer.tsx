      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-lg bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-out flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <div className="flex items-center gap-3">
            <button onClick={onClose} className="p-1 hover:bg-slate-100 rounded-lg text-slate-500 cursor-pointer">
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6" style={{ scrollbarWidth: "none" }}>
          ...
          {activeTab === "details" && (
            <>
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-3">
                    <input type="date" value={date} onChange={(e) => setDate(e.target.value)}
                      className="px-3 py-2 text-sm border border-slate-200 rounded-lg focus:border-blue-500 outline-none flex-1" />
                  ...
                  {!allDay && (
                    <div className="flex items-center gap-3">
                      <div className="flex items-center flex-1 border border-slate-200 rounded-lg overflow-hidden">
                        <span className="px-3 text-xs text-slate-500 bg-slate-50 border-r">Start</span>
                        <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)}
                          className="px-3 py-2 text-sm w-full outline-none" />
                      </div>
                      <div className="flex items-center flex-1 border border-slate-200 rounded-lg overflow-hidden">
                        <span className="px-3 text-xs text-slate-500 bg-slate-50 border-r">End</span>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 border border-slate-200 rounded-lg p-2 focus-within:border-blue-500">
          ...
          {activeTab === "tasks" && (
            <div className="space-y-4">
              <p className="text-sm text-slate-600">Assign specific tasks to be completed during this shift.</p>
              ...
              <ul className="space-y-2">
                {tasks.map((t, i) => (
                  <li key={i} className="flex justify-between items-center p-3 rounded-lg border border-slate-200 bg-slate-50 text-sm">
                    {t}
          ...
          {activeTab === "templates" && (
            <div className="text-center py-10">
              ...
              <p className="text-sm text-slate-500">Select a pre-built shift template to auto-fill these details.</p>
              <button className="mt-4 px-4 py-2 border border-slate-300 rounded-lg text-sm font-semibold hover:bg-slate-50">Create Template</button>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
          <div className="flex gap-2">
            ...
            <button onClick={() => { setStatus("Draft"); handleSave(); }}
              className="px-4 py-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 text-sm font-bold rounded-lg">
              Save Draft
            </button>
          </div>
          {editingShiftId && (
