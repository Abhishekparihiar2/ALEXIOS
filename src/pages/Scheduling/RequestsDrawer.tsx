...
export function RequestsDrawer({ isOpen, onClose }: RequestsDrawerProps) {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-[800px] bg-slate-50 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300">
        
        {/* Header */}
        <div className="flex flex-col px-6 py-5 bg-white border-b border-slate-200 shrink-0">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-slate-900">Schedule Requests</h2>
              <p className="text-sm text-slate-500 mt-1">Review time off, open shift claims, and shift replacement requests.</p>
            </div>
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center gap-3 shrink-0">
          <div className="relative flex-1 max-w-xs">
            ...
            <input 
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 transition-all shadow-sm"
            />
          </div>
          <select className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-sm font-medium text-slate-700 focus:outline-none focus:border-blue-500 shadow-sm">
          ...
          <button className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 text-sm font-semibold rounded-lg transition-colors shadow-sm ml-auto">
                ...
                <div key={req.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
                  <div className="p-5 flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-medium text-slate-500">{req.role} • {req.site}</p>
                        </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 p-4 bg-slate-50 rounded-lg border border-slate-100">
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-sm font-semibold text-slate-800">{new Date(req.startDate).toLocaleDateString()} - {new Date(req.endDate).toLocaleDateString()}</p>
                        </div>
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Duration</p>
                          <p className="text-sm font-semibold text-slate-800">{req.duration}</p>
                        </div>
                        <div>
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      ...
                      <button className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 text-red-600 hover:bg-red-50 text-sm font-semibold rounded-lg transition-colors shadow-sm">
                        <XCircle className="w-4 h-4" /> Decline
                      </button>
                      <button 
                        onClick={() => setExpandedDetails(expandedDetails === req.id ? null : req.id)}
                        className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 text-sm font-semibold rounded-lg transition-colors shadow-sm mt-2"
                      >
                  ...
                  {expandedDetails === req.id && (
                    <div className="p-5 border-t border-slate-100 bg-slate-50/50">
                      <h5 className="font-bold text-sm text-slate-800 mb-3">Audit Trail & Impact</h5>
                      <div className="text-sm text-slate-600 space-y-2">
                        <p><span className="font-semibold text-slate-800">Submitted:</span> {req.submittedDate}</p>
                        <p><span className="font-semibold text-slate-800">Schedule Impact:</span> Approving this will remove the employee from {req.affectedShifts} scheduled shifts.</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {activeTab === "claims" && (
            <div className="space-y-6">
              {MOCK_OPEN_SHIFT_CLAIMS.map(osc => (
                <div key={osc.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
                  <div className="p-4 border-b border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm font-medium text-slate-500 mt-1">{osc.site} • {new Date(osc.date).toLocaleDateString()} • {osc.time}</p>
                    </div>
                  </div>
                  
                  <div className="divide-y divide-slate-100">
                    {osc.claims.map(claim => (
                      <div key={claim.id} className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h5 className="font-bold text-slate-900">{claim.employeeName}</h5>
                        ...
                        <div className="flex gap-2 shrink-0">
                          <button className="px-4 py-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 text-sm font-semibold rounded-lg transition-colors shadow-sm">
                            Not Select
                          </button>
          ...
          {activeTab === "replacements" && (
            <div className="space-y-4">
              {MOCK_SHIFT_REPLACEMENTS.map(rep => (
                <div key={rep.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden p-5 flex flex-col md:flex-row items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h4 className="font-bold text-slate-900 text-lg mb-1">{rep.guardName}</h4>
                    <p className="text-sm font-medium text-slate-600 mb-3">{rep.position} • {rep.site}</p>
                    ...
                    <div className="grid grid-cols-2 gap-4 p-3 bg-slate-50 rounded-lg border border-slate-100">
                      <div>
                        ...
                        <p className="text-sm font-semibold text-slate-800">{new Date(rep.shiftDate).toLocaleDateString()} • {rep.shiftTime}</p>
                      </div>
                      <div>
                  ...
                  <div className="flex flex-col gap-2 shrink-0 w-full md:w-auto">
                    ...
                    <button className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 text-amber-600 hover:bg-amber-50 text-sm font-semibold rounded-lg transition-colors shadow-sm">
                      <RefreshCw className="w-4 h-4" /> Convert to Open Shift
                    </button>
                    <button className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-100 text-sm font-semibold rounded-lg transition-colors shadow-sm mt-2">
                      Decline
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-200 bg-slate-50">
              <h3 className="font-bold text-lg text-slate-900">Approve Time Off</h3>
              <p className="text-sm text-slate-500 mt-1">Review impact for {approvingTimeOff.employeeName}</p>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                <label className="text-sm font-bold text-slate-800">How should affected shifts be handled?</label>
                ...
                <label className="flex items-start gap-3 p-3 border border-slate-200 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors">
                  <input type="radio" name="shiftHandling" className="mt-1" />
                  <div>
                    <p className="text-sm font-bold text-slate-700">Keep assigned for manual replacement</p>
                    <p className="text-xs text-slate-500 mt-0.5">They will appear in the Shift Replacements tab.</p>
                  </div>
                </label>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-3">
              <button onClick={() => setApprovingTimeOff(null)} className="px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-200 rounded-lg transition-colors">Cancel</button>
      ...
      {replacingShift && (
        <div className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm flex flex-col justify-end sm:items-center sm:justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full h-[80vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
              <div>
                <h3 className="font-bold text-lg text-slate-900">Find Replacement</h3>
                <p className="text-sm text-slate-500 mt-1">Select an eligible guard for {replacingShift.position}</p>
              </div>
              <button onClick={() => setReplacingShift(null)} className="p-2 text-slate-400 hover:bg-slate-200 rounded-full"><X className="w-5 h-5" /></button>
            </div>
            
            <div className="p-4 bg-white border-b border-slate-200 flex items-center gap-3 shrink-0">
              <div className="relative flex-1">
                <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
                <input type="text" placeholder="Search eligible guards..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 transition-all" />
              </div>
            </div>
            
            <div className="flex-1 overflow-y-auto bg-slate-50 p-4 space-y-3 no-scrollbar">
              {[1, 2, 3].map((i) => (
                <div key={i} className="bg-white border border-slate-200 rounded-lg p-4 flex items-center justify-between gap-4 hover:shadow-sm transition-shadow">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-slate-200 text-slate-700 font-bold text-sm flex items-center justify-center">
                      G{i}
                    </div>
                    <div>
                      <h5 className="font-bold text-slate-900">Guard Name {i}</h5>
