...
function StatusChip({ status }: { status: string }) {
    ...
    return <span className="px-2.5 py-1 text-xs font-bold rounded-md border text-slate-500 bg-slate-100 border-slate-200">Inactive</span>;
}
    ...
    return (
        <div className="p-4 md:p-6 w-full max-w-[1600px] mx-auto h-full flex flex-col animate-in fade-in min-w-0 min-h-0 overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 mb-6 w-full shrink-0">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Groups & Segments</h1>
                    <p className="text-sm text-slate-500 mt-1">Create and manage user groups for operational organization and access.</p>
                </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 shrink-0 min-w-0">
                <div className="flex gap-2 items-center flex-wrap overflow-x-auto hide-scrollbar w-full">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Search groups..." className="w-full md:w-64 pl-9 pr-3 py-2.5 text-sm bg-white/80 border border-slate-200 rounded-xl outline-none focus:border-blue-600 transition-colors shadow-sm backdrop-blur-md font-medium" />
                    </div>
                    <button className="shrink-0 px-3 py-2.5 text-sm font-bold text-slate-600 bg-white/80 shadow-sm hover:bg-slate-50 border border-slate-200 rounded-xl flex items-center gap-1.5 transition-colors">
                            ...
                            {MOCK_GROUPS.map((g) => (
                                <tr key={g.id} onClick={() => onNavigate("detail", g)} className="hover:bg-slate-50/80 transition-colors cursor-pointer group">
                                    ...
                                    <td className="px-5 py-4 text-center">
                                        <span className="font-bold text-slate-700 bg-slate-100 px-3 py-1 rounded-full">{g.members}</span>
                                    </td>
                                    <td className="px-5 py-4 text-center">
        ...
        <div className="w-full h-full flex flex-col p-4 md:p-6 mx-auto max-w-4xl relative animate-in slide-in-from-right-2 overflow-hidden">
            <div className="flex items-center justify-between mb-6 shrink-0">
                <div className="flex items-center gap-4">
                    <button onClick={() => onNavigate("list")} className="p-2 text-slate-500 hover:bg-slate-100 rounded-xl transition-colors shrink-0">
                        <ChevronLeft className="w-5 h-5" />
                    </button>
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">{isEdit ? "Edit Group" : "Create Group"}</h1>
                        <p className="text-sm text-slate-500 mt-1">Configure operational collection of users for specific authorization logic.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto glass-panel p-6 lg:p-8 rounded-2xl border border-slate-200/60 bg-white/80 shadow-sm backdrop-blur-md hide-scrollbar">
                <div className="max-w-2xl space-y-6">
                    <div>
                        <label className="block text-sm font-bold text-slate-900 mb-1.5">Group Name <span className="text-red-500">*</span></label>
                        <input type="text" defaultValue={editData?.name} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:border-blue-600 transition-colors" placeholder="e.g. Emergency Response Team" />
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-slate-900 mb-1.5">Description</label>
                        <textarea rows={3} defaultValue={editData?.description} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium outline-none focus:border-blue-600 transition-colors" placeholder="Describe the purpose of this group..." />
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-slate-900 mb-1.5">Status <span className="text-red-500">*</span></label>
                    ...
                    <div className="pt-4 border-t border-slate-100">
                        <label className="block text-sm font-bold text-slate-900 mb-1.5">Members <span className="text-red-500">*</span></label>

                        {/* Member Search / Multi-Select */}
                        <div className="relative mb-3">
                            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                            <input type="text" placeholder="Search users by name..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-blue-600 transition-colors font-medium" />
                        </div>

                        <div className="border border-slate-200 rounded-xl bg-slate-50/50 p-3 max-h-[300px] overflow-y-auto space-y-1">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-1">Select Members</p>
                            {MOCK_MEMBERS.map((m) => (
                                <label key={m.id} className="flex items-start gap-3 p-2 rounded-lg hover:bg-slate-100 cursor-pointer border border-transparent hover:border-slate-200 transition-colors">
                                    <input type="checkbox" defaultChecked={isEdit} className="mt-1 w-4 h-4 text-blue-600 rounded border-slate-300" />
                                    <div>
                                        <div className="font-bold text-slate-900">{m.name}</div>
                                        <div className="text-xs font-semibold text-slate-500">{m.role} • {m.site}</div>
                                    </div>
                                </label>
                            ))}
                        </div>
                    ...
                    <div className="pt-6 flex gap-3">
                        <button onClick={() => onNavigate("list")} className="px-5 py-2.5 rounded-xl text-sm font-bold border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 shadow-sm transition-colors">
                            Cancel
                        </button>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 shrink-0 w-full">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium text-slate-500 mt-1 max-w-2xl">{group.description}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={() => onNavigate("edit", group)} className="flex items-center gap-2 px-4 py-2 text-sm font-bold border border-slate-200 rounded-xl bg-white text-slate-600 hover:bg-slate-50 transition-colors shadow-sm">
                        Edit Group
                    </button>
                    <button className="p-2 text-sm font-semibold border border-slate-200 rounded-xl bg-white text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
                        <MoreHorizontal className="w-5 h-5" />
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-6 w-full overflow-hidden">
                ...
                <div className="flex-1 glass-panel rounded-2xl border border-slate-200/60 bg-white/90 shadow-sm backdrop-blur-md flex flex-col overflow-hidden h-full">
                    <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
                        <div>
                            <h3 className="font-bold text-slate-900">Assigned Members</h3>
                            <p className="text-xs text-slate-500 font-medium">{group.members} total active entries</p>
                        </div>
                    ...
                    <div className="p-3 border-b border-slate-100 bg-white">
                        <div className="relative">
                            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                            <input type="text" placeholder="Search among members..." className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-blue-600 font-medium" />
                        </div>
                    </div>

                    <div className="flex-1 overflow-y-auto">
                        <table className="w-full text-left text-sm text-slate-600">
                            <thead className="bg-slate-50/80 text-xs uppercase text-slate-500 sticky top-0 border-b border-slate-200 font-bold">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {MOCK_MEMBERS.map(m => (
                                    <tr key={m.id} className="hover:bg-slate-50">
                                        <td className="px-5 py-3 font-bold text-slate-900">{m.name}</td>
                                        <td className="px-5 py-3 font-medium text-slate-600">{m.role}</td>
                                        <td className="px-5 py-3 text-slate-500">{m.site}</td>
                ...
                <div className="w-full md:w-[380px] shrink-0 glass-panel border border-slate-200/60 bg-white/80 rounded-2xl shadow-sm flex flex-col p-0 overflow-hidden h-full">
                    <div className="p-5 border-b border-slate-200 bg-slate-50/50">
                        <h3 className="font-bold text-slate-900">Group Activity</h3>
                    </div>
                    <div className="flex-1 overflow-y-auto p-5">
                        <div className="space-y-6">
                            <div className="flex gap-4">
                                <div className="mt-1 flex-shrink-0 w-2 h-2 rounded-full border-2 border-green-500 bg-white"></div>
                                <div>
                                    <p className="text-sm font-bold text-slate-900">Sarah Jenkins added to group</p>
                                    <p className="text-xs text-slate-500 font-medium">Aug 5, 2026 • 2:40 PM</p>
                                </div>
                            </div>
                            <div className="flex gap-4">
                                <div className="mt-1 flex-shrink-0 w-2 h-2 rounded-full border-2 border-blue-500 bg-white"></div>
                                <div>
                                    <p className="text-sm font-bold text-slate-900">Group description updated</p>
                                    <p className="text-xs text-slate-500 font-medium">Aug 4, 2026 • 10:15 AM</p>
                                </div>
                            </div>
                            <div className="flex gap-4">
                                <div className="mt-1 flex-shrink-0 w-2 h-2 rounded-full border-2 border-slate-400 bg-white"></div>
                                <div>
                                    <p className="text-sm font-bold text-slate-900">Group created</p>
                                    <p className="text-xs text-slate-500 font-medium">Aug 1, 2026 • System Admin</p>
                                </div>
                            </div>
                        </div>
                    </div>
